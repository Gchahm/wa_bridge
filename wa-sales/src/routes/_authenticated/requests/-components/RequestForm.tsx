...
interface RequestFormProps {
  customerId: string
  chatId?: string | null
  /** The existing request being edited, or null when creating a new one. */
  request: FlightRequest | null
  /** Called after a successful update (edit flow). */
  onSaved: () => void
  /** Called after a successful insert (create flow) with the newly created row. */
  onCreated: (created: FlightRequest) => void
  /**
   * Called when the user cancels:
   * - In edit flow: goes back to the summary view.
   * - In create flow: closes the sheet.
   */
  onCancel: () => void
}
  ...
  const form = useForm({
    ...
    onSubmit: async ({ value }) => {
      ...
      if (request?.id) {
        ...
        onCreated(data as unknown as FlightRequest)
      }
    },
  })
